...
def save_pdf(link,csrfmiddlewaretoken,blur_img_1_path,blur_img_2_path,results,not_found_using_nlp_and_img=None):
    ...
    if blur_img_1_path:
        method = "Generated using Image Processing"
    else:
        method = "Generated using NLP and Image Processing" if not_found_using_nlp_and_img else "Generated using NLP"
    ...
    if blur_img_1_path:
        ...
        pdf.ln()
        # images
        pdf.line(x1=195, y1=120, x2=10, y2=120)
        pdf.ln()
        pdf.cell(70,12,'Image - 1',align = 'R')
        pdf.ln()
        pdf.cell(70,12,"P-content: 57%",align="R")
        pdf.ln()
        pdf.cell(70,12,"S-content: 57%",align="R")
        pdf.ln()

        img1 = Image.open(blur_img_1_path)
        size=(200,200)
        img1 = img1.resize(size)
        pdf.image(img1, x = 90, y = 125)
        pdf.ln()

        pdf.line(x1=195, y1=205, x2=10, y2=205)
...
def generate_report(link,csrfmiddlewaretoken,results,is_nlp=None):
    ...
    if is_nlp:
        # this means it's nlp based generated report:
        print("Report Generated by NLP")
        report.img_1 = None
        report.img_2 = None
        report.zip = None
        report.result = True
        pdf_report = save_pdf(link,csrfmiddlewaretoken,None,None,True)
    ...
    if len(results) == 0:
         ...
         report.zip = None
         report.result = False
         pdf_report = save_pdf(link,csrfmiddlewaretoken,None,None,results,not_found_using_nlp_and_img=True)
         report.pdf_report = pdf_report
